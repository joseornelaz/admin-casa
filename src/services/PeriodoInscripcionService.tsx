import { useQuery } from "@tanstack/react-query";
import { apiClient } from "./ApiConfiguration/httpClient";
import { PERIODOS_INSCRIPCION } from "../types/endpoints";
import type { PeriodoInscripcionPayload } from "../types/PeriodoInscripcion.interface";
import { PeriodoInscripcionMock } from "../mockdata/PeridoInscripcionMock";

export const GetPeriodosInscripcion = (options?: { enabled?: boolean }) => {
    return useQuery<any, Error>({
        queryKey: ['periodosInscripcion'],
        queryFn: () => Promise.resolve(PeriodoInscripcionMock),
        staleTime: 1000 * 60 * 10, // 10 minutos de stale time
        ...options
    });

    // return useQuery<PeriodoInscripcion, Error>({
    //     queryKey: [PERIODOS_INSCRIPCION.GET_PERIODOS_INSCRIPCION.key],
    //     queryFn: () => apiClient.get<PeriodoInscripcion>(`${PERIODOS_INSCRIPCION.GET_PERIODOS_INSCRIPCION.path}`),
    //     staleTime: 1000 * 60 * 10, // 10 minutos de stale time
    //     ...options
    // });
}

export const GetPeriodoById = (id: number, options?: { enabled?: boolean }) => {
    return useQuery<any, Error>({
        queryKey: ['periodo', id],
        queryFn: () => Promise.resolve(PeriodoInscripcionMock.find(grupo => grupo.id === id)),
        staleTime: 1000 * 60 * 10, // 10 minutos de stale time
        ...options
    });
}

export const PostRegistrarPeriodo = async (payload: PeriodoInscripcionPayload) => {
    return { success: true, data: payload };

    const encryptedPayload = await apiClient.encryptData({ ...payload });
    return await apiClient.post(PERIODOS_INSCRIPCION.POST_REGISTRAR_PERIODO.path, { data: encryptedPayload });
}